...
static struct ina2xx_data *ina2xx_update_device(struct device *dev)
{
        ...
        if (time_after(jiffies, data->last_updated +
                ...
                for (i = 0; i < data->config->registers; i++) {
                        int rv = swab16((i2c_smbus_read_word_data(client, i)));
                        if (rv < 0) {
                                ret = ERR_PTR(rv);
                                goto abort;
                        }
...
//      long shunt = 10000; /* default shunt value 10mOhms */
        ...
        i2c_smbus_write_word_data(client, INA2XX_CONFIG,
                                     swab16(data->config->config_default));
        /* set current LSB to 1mA, shunt is in uOhms */
        /* (equation 13 in datasheet) */
        i2c_smbus_write_word_data(client, INA2XX_CALIBRATION,
                                     swab16(data->config->calibration_factor / shunt));
